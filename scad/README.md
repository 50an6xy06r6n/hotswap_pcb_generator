...
| `additional_plate_cutouts()` | Unlike most of the other parameters, this is a module, not a variable. You can use it to define custom cutouts in the plate (also applies to cases). Use `position_item()` from `utils.scad` to help place the cutouts relative to existing components. The sample `eggcat.scad` uses this to create a large cutout above the MCU and TRRS. | `layout.scad` |
| `additional_case_cavities()` | This is another module parameter. You can use it to add to the internal case cavity to accommodate wire routing, batteries, etc. Use `position_item()` from `utils.scad` to help place the cutouts relative to existing components. The sample `eggcat.scad` uses this to create a relief on one side of the case that's used for wire routing off the MCU. | `layout.scad` |
| `backplate_cutouts()` | This is another module parameter. You can use it to add pockets and cutouts to the bottom of the backplate (useful for positioning bumpons and mounting hardware). Use `project_onto_plane()` from `utils.scad` to help place the cutouts relative to existing components. This will account for the tent angle of your backplate and ensure that the feature is still positioned according to the x-y coordinates of the main layout. The sample `eggcat.scad` uses this to create pockets for bumpons and a magsafe adapter ring. | `layout.scad` |
...
| `backplate_lip_width` | If using the `plate_case` case type, this defines the width of the feature that indexes into the bottom of the case. If set to `undef` it becomes a solid area. | `parameters.scad` |
| `backplate_screw_flange_thickness` | Defines the thickness of the material that backplate standoff screws clamp onto (i.e. thickness - counterbore depth). | `parameters.scad` |
